        ...
        function CommentGetData() {
            ...
            .then(data => {
                ...
                data.forEach(childSnapshot => {
                    ...
                    const formattedTime = `${day} ${hours}:${minutes < 10 ? '0' + minutes : minutes}`;
                    const commentDiv = document.createElement("div");
                    commentDiv.innerHTML = `<strong>${sender}:</strong> ${comment}<br><br> ${formattedTime}`;
                    bookText.appendChild(commentDiv);
                });
            })
            .catch(err => { alert(err) });
        }
